...
chcp 65001 >nul
REM ============================================================================
REM Task Scheduler entry point for the morning batch (race cards).
REM
...
set LOGFILE=logs\morning-%LOGDATE%.log

echo ---------------------------------------------------------------- >> "%LOGFILE%"
echo [%DATE% %TIME%] morning batch start >> "%LOGFILE%"
...
pushd "%~dp0bin\Debug\net48"
"%EXE%" morning >> "%~dp0%LOGFILE%" 2>&1
set EXITCODE=%ERRORLEVEL%
popd

echo [%DATE% %TIME%] morning batch end (exit=%EXITCODE%) >> "%LOGFILE%"
exit /b %EXITCODE%
