...
地方競馬（UmaConn）は、成績レコード(SE)の一部項目を仕様書どおりの初期値のまま返します。
バイト位置のズレではないことは確認済み（同レコード内の着差コード=343バイト目が正しく読めるため、
その後ろの単勝オッズ=360・人気順=364・後3ハロン=391もズレていない）。

| 項目 | 地方競馬(UmaConn) | 対応 |
| --- | --- | --- |
| 単勝オッズ | SEでは初期値 | 速報オッズ `0B31` のO1レコードから補完（実装済み） |
| 単勝人気順 | SEでは初期値 | 同上 |
| 後3ハロンタイム | SEでは初期値 | **取得不可**（他のデータ種別にも該当項目が無い） |

中央競馬（JV-Link）はSEレコードにこれらの実値が入るため、補完処理は動きません
（`RaceResultService` はSE側に値がある項目を上書きしない）。

競馬場コード83（ばんえい競馬と推定）は、負担重量・馬体重が通常競馬と桁が異なり
3バイトの数値枠に収まらないため値が想定外になります。通常の地方競馬場（盛岡35・
船橋43・金沢46 等）は正常です。
